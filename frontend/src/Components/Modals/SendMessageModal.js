...
const SendMessageModal = ({receiverId}) => { 
    ...
    const makeOffer = async (id) => { 
        try {
            const response = await fetch('/api/messages/send/new', {
                ...
                body: JSON.stringify({ message, listingId: null, receiverId })
            })
    ...
    return (
        <>
            <button 
                className="px-10 py-2 text-lg font-semibold text-center text-white border rounded bg-violet-700 h-fit w-fit"
